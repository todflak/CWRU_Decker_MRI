function out = CWRU_spm_dicom_convert(hdr,opts,root_dir,format,out_dir, ...
               ForceMosaic, ImageCountInMosaic)
% Modified to deal with mosaic ASL images that are lacking the correct 
% header info!  TF 19 Feb 2018
...
if (exist('ForceMosaic','var')==0) || (isempty(ForceMosaic))
   ForceMosaic = false;
end 
if (exist('ImageCountInMosaic','var')==0) || (isempty(ImageCountInMosaic))
   ImageCountInMosaic = 0;
end 
...
[mosaic, standard]  = select_mosaic_images(images, ForceMosaic );
...
if (strcmp(opts,'all') || strcmp(opts,'mosaic')) && ~isempty(mosaic)
    fmos = convert_mosaic(mosaic,root_dir,format,out_dir, ImageCountInMosaic);
end
...
%==========================================================================
function fnames = convert_mosaic(hdr,root_dir,format,out_dir, ImageCountInMosaic)
...
for i=1:length(hdr)
    ...
    dim      = [0 0 0];
    if (ImageCountInMosaic>0)  
       dim(3)   = ImageCountInMosaic;
    else
...
%==========================================================================
function [mosaic,standard] = select_mosaic_images(hdr, ForceMosaic)
mosaic   = {};
standard = {};
for i=1:length(hdr)
    if (ForceMosaic)  
...
%==========================================================================
function nrm = read_SliceNormalVector(hdr)
if (isfield(hdr,'CSAImageHeaderInfo'))
