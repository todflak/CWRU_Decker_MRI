%  The goal of this function is to produce mean perfusion/CBF for grey
%  matter, white matter, CSF.
%  The strategy is to use SPM segmentation to warp the data into a standard
%  TPM (tissue probability map), and then simply summarize that data.
%  Tod Flak 28 Aug 2017


function CWRU_ASL_SimpleSegmentation(Data_Root, Study_Folders_ToProcess, LogFilename, ...
                  StructuralImageNameSuffix, ImageToSummarizeFilename, MaskForSummarizeFilename, OutputFileRootName)
...
fprintf(fidLog,'\n%s: CWRU_ASL_SimpleSegmentation, starting processing.\n', datestr(datetime('now')));
...
for s = 1:PAR.nsubs
   ...
   [CodePath,nam,ext,num] = spm_fileparts(mfilename('fullpath'));
   ...
   fprintf(fidLog,'%s: For subject #%d (''%s''), completed segmentation, produced the class images for tissues 1-5 \n',  ...
...
fprintf(fidLog,'%s: CWRU_ASL_SimpleSegmentation, completed processing.\n', datestr(datetime('now')));
if (fidLog>1) , fclose(fidLog); end
clear matlabbatch fidLog
